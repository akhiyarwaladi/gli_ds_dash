...
def get_vp():
    ...
    [{'label': x, 'value': x} for x in vp.columns[1:]]
	
def get_sp():
	sp = pd.read_csv('/home/server/gli-data-science/akhiyar/out_plot/general_moengage/search_product.csv')
	sp['All'] = sp.sum(axis=1)

	return sp, \
    round(sp.mean(axis=1).mean(), 2), \
    [{'label': x, 'value': x} for x in sp.columns[1:]]
